...
- [ ] **Export script.** Pull photos plus metadata (store, date, rep, visit) out of MongoDB to disk, read-only, in batches. — *`configs/export.yaml` now points at the real `atpg` schema, the read-only user, and filters to `photo_type: shelf`; still needs the `location` region join (§1) before a full run.*

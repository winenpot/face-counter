...
- [ ] **Export script.** Pull photos plus metadata (store, date, rep, visit) out of MongoDB to disk, read-only, in batches. — *`configs/export.yaml` now points at the real `atpg` schema, the read-only user, filters to `photo_type: shelf`, and joins `location` for region. Code is ready; only a full run against production is left.*
- [ ] **Manifest.** One CSV row per photo: id, store, visit, date, file path, image size. Every dataset later is built from it. — *blocked on running the full export; `city` is now filled from the `location` join (99% match rate), `visit_id`/`rep_id` stay empty, they don't exist on the photos.*

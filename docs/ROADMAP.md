...
- [ ] **Export script.** Pull photos plus metadata (store, date, rep, visit) out of MongoDB to disk, read-only, in batches. — *`configs/export.yaml` now points at the real `atpg` schema and the read-only user; still needs `photo_type: shelf` filtering (§1) and the `location` region join (§2) before a full run.*
